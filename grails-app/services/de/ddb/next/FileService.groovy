...
class FileService {
    ...
    def downloadFile(String address) {
        def fileUrl=new UrlResource(address).getURL()
    ...
    def String getContentType(byte[] fileDataSource, String name)
    {
        ...
        System.arraycopy(fileDataSource, 0, header, 0, Math.min(fileDataSource.length, header.length));
