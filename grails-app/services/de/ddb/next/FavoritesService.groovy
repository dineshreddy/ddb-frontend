...
class FavoritesService {
    ...
    def private createFavoritesLinkNavigation(offset,rows,order,by,folderId){
        ...
        return g.createLink(controller:'favorites', action: 'favorites',params:[(SearchParamEnum.OFFSET.getName()):offset,(SearchParamEnum.ROWS.getName()):rows,order:order,by:by,id:folderId])
    }
    ...
    def private createPublicFavoritesLinkNavigation(Integer offset, Integer rows, String order, String userId, String folderId, String by){
        def g = grailsApplication.mainContext.getBean('org.codehaus.groovy.grails.plugins.web.taglib.ApplicationTagLib')
        return g.createLink(controller:'favorites', action: 'publicFavorites', params:[userId: userId, folderId: folderId, (SearchParamEnum.OFFSET.getName()):offset, (SearchParamEnum.ROWS.getName()):rows, order:order, by:by])
    }
