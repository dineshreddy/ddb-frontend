          ...
          <div class="thumbnail" id="thumbnail-${entityItem.id}">
            <g:link class="persist" controller="institution" action="showInstitutionsTreeByItemId" params="${params + [id:entityId]}">
              <g:if test="${entityItem.preview.thumbnail!="null"}">
                <img src="${request.getContextPath() + entityItem.preview.thumbnail}" alt="<ddb:getWithoutTags>${entityItem.preview.title}</ddb:getWithoutTags>"/>
              </g:if>
              <g:else>
          ...
          <div class="item-options bl">
            <ul class="item-options-ul">
              <ddbcommon:isLoggedIn>
                <li>
                  <div data-itemid="${entityItem.id}" data-objecttype="${Type.INSTITUTION.name}" data-actn="POST" class="add-to-favorites"
