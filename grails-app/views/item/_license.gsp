  ...
  <div class="fields">
    <div class="row">
      ...
      <div class="value <g:if test="${display}">span4</g:if><g:else>span10</g:else>">
        <a href="${license.url}" target="_blank" class="no-external-link-icon"><g:if test="${license.img}"><img src="${license.imgContext + license.img}" class="license-icon" alt="${g.message(code:"ddbnext.License_Field")}"/></g:if><span>${license.text}</span></a>
      </div>
    </div>
  </div>
</g:if>
